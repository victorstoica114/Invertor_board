...
| `pylon_rs485` | `datasheets/RS485-protocol-pylon-low-voltage-V3.3-20180821.pdf`, firmware Pylon bridge | ASCII frame parsing, length/checksum, command/response tracking, request selectors, common `0x61`, zero/non-zero `0x62` flags, `0x63` charge/discharge status, and `0x42` simple/pack cell-info payloads where present | Some Pylon-compatible BMS profiles answer `0x42` with OK/no-payload or reject it; keep those as protocol behavior, not decoder failure. |
