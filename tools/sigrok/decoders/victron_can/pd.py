##
## Victron-compatible CAN protocol decoder for PulseView/libsigrokdecode.
##
## This decoder is standalone on a logic CAN RX signal. It reuses the built-in
## CAN decoder and adds Victron-specific annotations for decoded frames. It can
...
try:
    from .victron_can import FRAME_NAMES, VERSION, describe_packet, format_data, frame_summary
except Exception:
    from victron_can import FRAME_NAMES, VERSION, describe_packet, format_data, frame_summary
...
class Ann:
    VICTRON_FRAME = len(CanDecoder.annotations) if CanDecoder is not None else 0
    VICTRON_PAYLOAD = VICTRON_FRAME + 1
    VICTRON_DECODED = VICTRON_FRAME + 2
    VICTRON_WARNING = VICTRON_FRAME + 3
...
class Decoder(BaseDecoder):
    api_version = 3
    id = 'victron_can'
    name = 'Victron CAN {}'.format(VERSION)
    longname = 'Victron-compatible low-voltage CAN {}'.format(VERSION)
    desc = 'Victron/Pylon-like low-voltage BMS frames over Classic CAN.'
    license = 'gplv2+'
    inputs = ['logic']
    outputs = ['victron_can']
    ...
    if CanDecoder is not None:
        annotations = CanDecoder.annotations + (
            ('victron-frame', 'Victron frame'),
            ('victron-payload', 'Victron payload'),
            ('victron-decoded', 'Victron decoded value'),
            ('victron-warning', 'Victron warning'),
        )
        annotation_rows = CanDecoder.annotation_rows + (
            ('victron-frames', 'Victron Frames', (Ann.VICTRON_FRAME,)),
            ('victron-payloads', 'Victron Payload', (Ann.VICTRON_PAYLOAD,)),
            ('victron-decoded-values', 'Victron Decoded', (Ann.VICTRON_DECODED,)),
            ('victron-warnings', 'Victron Warnings', (Ann.VICTRON_WARNING,)),
        )
    else:
        annotations = (
            ('victron-warning', 'Victron warning'),
    ...
    def put_victron_ann(self, ss, es, ann, texts):
    ...
    def putpy(self, data):
        ...
        if rtr_type not in (None, 'Data', 'data', 0):
            self.put_victron_ann(self.ss_packet, self.es_packet, Ann.VICTRON_WARNING,
                                 ['Remote frame 0x{:03X} ignored'.format(can_id), 'RTR ignored'])
            return

        packet = (frame_type, can_id, rtr_type, dlc, payload)
        self.put_victron_ann(self.ss_packet, self.es_packet, Ann.VICTRON_FRAME,
                             [frame_summary(packet)])
        self.put_victron_ann(self.ss_packet, self.es_packet, Ann.VICTRON_PAYLOAD,
                             [format_data(payload), 'DATA'])
        self.put_victron_ann(self.ss_packet, self.es_packet, Ann.VICTRON_DECODED,
