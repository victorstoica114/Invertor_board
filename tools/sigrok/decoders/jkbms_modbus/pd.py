##
## JKBMS RS485 Modbus RTU protocol decoder for PulseView/libsigrokdecode.
##
## Stack this decoder above the built-in UART decoder. Use 115200 8N1 for JK
## app profile 001, or 9600 8N1 for profile 013.
##
...
class Decoder(srd.Decoder):
    ...
    options = (
        {'id': 'inter_frame_gap_us', 'desc': 'Inter-frame gap (us)', 'default': 2000},
    )
    ...
    def maybe_flush_on_gap(self, rxtx, ss):
        ...
        gap_us = int(options.get('inter_frame_gap_us', 2000))
        if gap_us <= 0:
            return
    ...
    def finish_frame(self, rxtx, es):
        ...
        try:
            ...
            self.put_ann(ss, es, Ann.WARNING,
                         ['Invalid JKBMS/Modbus frame: {} [{}]'.format(exc, hex_bytes(raw)),
    ...
    def decode(self, ss, es, data):
        ...
        if len(self.buf[rxtx]) > 260:
            self.put_ann(self.pos[rxtx][0][0], es, Ann.WARNING,
                         ['JKBMS/Modbus frame too long, dropping', 'Too long'])
            self.reset_direction(rxtx)
