##
## Deye-compatible low-voltage CAN frame helpers.
##
...
def describe_351(data):
    return '0x351 limits chgV={:.1f}V chgI={:.1f}A disI={:.1f}A lowV={:.1f}V'.format(
        le16(data, 0) / 10.0,
        le16(data, 2) / 10.0,
        le16(data, 4) / 10.0,
        le16(data, 6) / 10.0,
...
def describe_359(data):
    ...
    return '0x359 module info {}'.format(' '.join(parts))


def describe_35c(data):
    state = data[0] if data else 0
    return '0x35C state raw=0x{:02X} charge={} discharge={} balance={} raw={}'.format(
        state,
        on_off(state & 0x80),
        on_off(state & 0x40),
        on_off(state & 0x20),
...
def describe_35e(data):
    ...
    if text:
        return "0x35E identity '{}' raw={}".format(text, format_data(data))
    return '0x35E identity raw={}'.format(format_data(data))


def describe_370(data):
    return '0x370 temp max={:.1f}C min={:.1f}C cell_max={:.3f}V cell_min={:.3f}V'.format(
...
def describe_371(data):
    return '0x371 temp_max_sensor={} temp_min_sensor={} cell_max_idx={} cell_min_idx={}'.format(
        le16(data, 0),
        le16(data, 2),
        le16(data, 4),
        le16(data, 6),
...
def frame_summary(can_packet):
    frame = can_data_bytes(can_packet)
    name = FRAME_NAMES.get(frame['id'], 'Deye?')
    return 'Deye CAN {} 0x{:03X} {} DLC={} [{}]'.format(
        VERSION,
        frame['id'],
        name,
        frame['dlc'],
