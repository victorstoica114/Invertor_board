...
void canForwardSnifferStart(const bridge_runtime_settings_t *settings)
{
    ...
    if (bmsOnCan && invOnCan) {
        ...
        bmsToInv.rawLogEnabled = bmsCanPylon || bmsCanDeye || bmsCanGoodwe || bmsCanSofar || bmsCanSma || bmsCanVictron;
        ...
        invToBms.rawLogEnabled = invCanPylon || invCanDeye || invCanGoodwe || invCanSofar || invCanSma || invCanVictron;
                     ...
                     ((settings->bms_protocol == PROTOCOL_CAN_DEYE) ? "CAN_DEYE" :
                      ((settings->bms_protocol == PROTOCOL_CAN_GOODWE) ? "CAN_GOODWE" :
                       ((settings->bms_protocol == PROTOCOL_CAN_SOFAR) ? "CAN_SOFAR" :
                        ((settings->bms_protocol == PROTOCOL_CAN_SMA) ? "CAN_SMA" :
                         ((settings->bms_protocol == PROTOCOL_CAN_VICTRON) ? "CAN_VICTRON" : "CAN_GROWATT"))))));
        }
        return;
    }
    ...
    if (bmsOnCan) {
        ...
        bmsSniff.rawLogEnabled = bmsCanPylon || bmsCanDeye || bmsCanGoodwe || bmsCanSofar || bmsCanSma || bmsCanVictron;
    ...
    if (invOnCan) {
        ...
        invSniff.rawLogEnabled = invCanPylon || invCanDeye || invCanGoodwe || invCanSofar || invCanSma || invCanVictron;
