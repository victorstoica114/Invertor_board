...
static bool validateSettings(const bridge_runtime_settings_t *s)
{
    ...
    if (s->bms_protocol < PROTOCOL_CAN_GROWATT || s->bms_protocol > PROTOCOL_RS485_VOLTRONIC) {
        return false;
    }
    if (s->inverter_protocol < PROTOCOL_CAN_GROWATT || s->inverter_protocol > PROTOCOL_RS485_VOLTRONIC) {
        return false;
    }
