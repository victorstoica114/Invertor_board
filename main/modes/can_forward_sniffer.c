...
void canForwardSnifferStart(const bridge_runtime_settings_t *settings)
{
    ...
    if (bmsOnCan && invOnCan) {
        ...
        if ((settings->mode == MODE_BRIDGE) && sameCanProtocol) {
            ESP_LOGI(EXAMPLE_TAG,
                     ...
                     canProtocolLabel(settings->bms_protocol));
        }
        return;
    }
    ...
    if (bmsOnCan) {
        ...
        if (bmsCanPylon) {
            ...
            ESP_LOGI(EXAMPLE_TAG, "CAN Victron diagnostic logging enabled on BMS side (%s[P%d])",
                     bmsSniff.rxName,
                     settings->bms_port);
        } else if (bmsCanJkbms250k) {
            ESP_LOGI(EXAMPLE_TAG, "JK BMS CAN 250K diagnostic logging enabled on BMS side (%s[P%d])",
                     bmsSniff.rxName,
                     settings->bms_port);
        } else if (bmsCanDaly) {
            ESP_LOGI(EXAMPLE_TAG, "Daly CAN diagnostic logging enabled on BMS side (%s[P%d])",
                     bmsSniff.rxName,
                     settings->bms_port);
        }
    }
    ...
    if (invOnCan) {
        ...
        invSniff.rawLogEnabled = CAN_DECODER_SHOW_RAW_FRAMES &&
                                 (invCanPylon || invCanDeye || invCanGoodwe || invCanSofar || invCanSma || invCanVictron);
