...
static esp_err_t rootHandler(httpd_req_t *req)
{
    static const char html[] =
        ...
        "const STALE_MS=parseInt('" STR(WEB_TELEMETRY_STALE_MS) "',10);"
        ...
        "if((t.protocol!=='JKBMS_MODBUS'&&t.protocol!=='JKBMS_RS485_NATIVE')||!hasData){return '';}"
        "const rows=["
        ...
        "];"
        "if(t.protocol==='JKBMS_MODBUS'){rows.push(row('Precharge',isNum(t.precharge_state)?String(t.precharge_state):'-'));}"
        ...
        "const stale=Boolean(t.stale)||(age>STALE_MS&&age!==0);"
        ...
        "const bmsCanProtoOpts=[{value:1,label:'CAN_GROWATT (CAN BMS)'},{value:4,label:'CAN_PYLON'},{value:5,label:'CAN_DEYE'},{value:7,label:'CAN_GOODWE'},{value:8,label:'CAN_SOFAR'},{value:9,label:'CAN_SMA'},{value:10,label:'CAN_VICTRON'}];"
        "const bmsRsProtoOpts=[{value:2,label:'RS485_GROWATT (Modbus Poller)'},{value:6,label:'JKBMS_MODBUS (RS485 Poller)'},{value:12,label:'JKBMS_RS485_NATIVE (Native Poller)'},{value:11,label:'PACE_RS485_MODBUS_V1.3 (Modbus Poller)'},{value:13,label:'VOLTRONIC_MODBUS / JK 007 (RS485 Poller)'},{value:14,label:'CHINA_TOWER_MODBUS / JK 008 (RS485 Poller)'},{value:15,label:'WOW_MODBUS / JK 009 (RS485 Poller)'},{value:3,label:'RS485_PYLON'}];"
        ...
        "else if(bl===2&&(bp===6||bp===12)&&il===2&&ip===2){txt='Special route active: '+protoLabel(bp)+' (RS485_'+bport+') -> RS485_GROWATT responder (RS485_'+iport+').';}"
        "else if(bl===2&&(bp===6||bp===12)&&il===2&&ip===3){txt='Special route active: '+protoLabel(bp)+' (RS485_'+bport+') -> RS485_PYLON responder (RS485_'+iport+').';}"
        "else if(bl===2&&bp===11&&il===2&&ip===3){txt='Special route active: PACE_RS485_MODBUS_V1.3 (RS485_'+bport+') -> RS485_PYLON responder (RS485_'+iport+').';}"
        "else if(bl===2&&bp===2&&il===2&&ip===3){txt='Special route active: RS485_GROWATT / JK 006 (RS485_'+bport+') -> RS485_PYLON responder (RS485_'+iport+').';}"
        "else if(bl===2&&bp===13&&il===2&&ip===3){txt='Special route active: VOLTRONIC_MODBUS / JK 007 (RS485_'+bport+') -> RS485_PYLON responder (RS485_'+iport+').';}"
        "else if(bl===2&&bp===14&&il===2&&ip===3){txt='Special route active: CHINA_TOWER_MODBUS / JK 008 (RS485_'+bport+') -> RS485_PYLON responder (RS485_'+iport+').';}"
        "else if(bl===2&&bp===15&&il===2&&ip===3){txt='Special route active: WOW_MODBUS / JK 009 (RS485_'+bport+') -> RS485_PYLON responder (RS485_'+iport+').';}"
        ...
        "else if(bl===1&&bp===4&&il===2&&ip===3){txt='Special route active: CAN_PYLON -> RS485_PYLON translator.';}"
        "else if(bl===2&&bp===3&&il===2&&ip===3){txt='Special route active: RS485_PYLON bridge (bidirectional pass-through + cache).';}"
