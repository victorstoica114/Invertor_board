...
static esp_err_t rootHandler(httpd_req_t *req)
{
    static const char html[] =
        ...
        "const canProtoOpts=[{value:1,label:'CAN_GROWATT (CAN BMS)'},{value:4,label:'CAN_PYLON'},{value:5,label:'CAN_DEYE'}];"
        "const rsProtoOpts=[{value:2,label:'RS485_GROWATT (Modbus Poller)'},{value:3,label:'RS485_PYLON'}];"
        ...
        "function protoLabel(v){const all=canProtoOpts.concat(rsProtoOpts);const p=all.find(o=>String(o.value)===String(v));return p?p.label:String(v);}"
