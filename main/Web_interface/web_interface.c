...
static esp_err_t rootHandler(httpd_req_t *req)
{
    static const char html[] =
        ...
        "];"
        "const actions='<div class=\"actions\"><button onclick=\"saveSettings()\">Apply</button><span id=\"settingsStatus\" class=\"mono\"></span></div><div id=\"routeHint\" class=\"mono\" style=\"margin-top:8px;color:#8aa0b7\"></div>';"
        ...
        "let res=null;"
        "try{res=await fetch('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload),signal:ctrl.signal});}"
        "finally{clearTimeout(timer);}"
        "const body=await res.json();"
        "status.textContent=body.message||'done';"
        "if(res.ok&&body.ok){setTimeout(loadSettings,300);}"
        "}catch(e){"
        "status.textContent='Save failed: '+(e&&e.name==='AbortError'?'timeout':(e&&e.message?e.message:String(e)));"
        "}"
        "}"
