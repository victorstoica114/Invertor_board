...
static esp_err_t startBmsTask(protocol_id_t protocol, QueueHandle_t outQueue)
{
    switch (protocol) {
        ...
        case PROTOCOL_ID_DALY: {
            ...
            if (settings.bms_line == LINE_CAN && bridgeProtocolIsCanDaly(settings.bms_protocol)) {
                return dalyCanBmsTaskStart(outQueue, &settings);
            }
...
static bool isRsPylonToCanGrowattRoute(const bridge_runtime_settings_t *settings)
{
    if (settings == NULL) {
        return false;
    }
...
static bool isRsJkbmsToCanGrowattRoute(const bridge_runtime_settings_t *settings)
{
    if (settings == NULL) {
        return false;
    }
...
static bool isCanDalyToCanPylonRoute(const bridge_runtime_settings_t *settings)
{
    if (settings == NULL) {
        return false;
    }
...
esp_err_t orchestratorStartFromRuntime(const bridge_runtime_settings_t *settings)
{
    ...
    const bool canDalyToCanPylon = isCanDalyToCanPylonRoute(settings);
    ...
    if (rsPylonToRsGrowatt) {
        ...
        memset(&g_orchestratorCtx, 0, sizeof(g_orchestratorCtx));
        g_orchestratorCtx.bmsProtocol = PROTOCOL_ID_PYLON;
        g_orchestratorCtx.inverterProtocol = PROTOCOL_ID_GROWATT;

        canDecoderResetCaches();
        pylonRs485BridgeEnable();

        esp_err_t err = jkbmsRs485GrowattBridgeEnable(rsUartByPort(settings->inverter_port),
                                                      rsDirByPort(settings->inverter_port),
                                                      rsNameByPort(settings->inverter_port));
        if (err != ESP_OK) {
            ESP_LOGW(EXAMPLE_TAG,
                     "Pylon RS485->RS485 Growatt route failed to start responder (err=0x%x)",
                     (unsigned)err);
        ...
        g_orchestratorCtx.canRs485TranslatorActive = true;
        ESP_LOGI(EXAMPLE_TAG,
                 "Orchestrator started Pylon RS485->RS485 Growatt route: BMS(RS485_%u) -> Inverter(%s:%u)",
                 (unsigned)settings->bms_port,
                 rsNameByPort(settings->inverter_port),
                 (unsigned)settings->inverter_port);
        return ESP_OK;
    ...
    if (rsPylonToCanGrowatt) {
        ...
        esp_err_t err = growattInverterTaskStart(g_orchestratorCtx.inverterQueue);
        if (err != ESP_OK) {
            ESP_LOGW(EXAMPLE_TAG,
                     "Pylon RS485->CAN Growatt route failed to start sender (err=0x%x)",
                     (unsigned)err);
            pylonRs485BridgeStop();
            orchestratorReset(&g_orchestratorCtx);
            return err;
        ...
        g_orchestratorCtx.canRs485TranslatorActive = true;
        ESP_LOGI(EXAMPLE_TAG,
                 "Orchestrator started Pylon RS485->CAN Growatt route: BMS(RS485_%u) -> Inverter(%s:%u)",
                 (unsigned)settings->bms_port,
    ...
    if (rsJkbmsToCanGrowatt) {
        ...
        if (err != ESP_OK) {
            ESP_LOGW(EXAMPLE_TAG,
                     "JKBMS RS485 BMS task failed for RS485->CAN Growatt route (err=0x%x)",
                     (unsigned)err);
            orchestratorReset(&g_orchestratorCtx);
            return err;
        }
        ...
        canDecoderResetCaches();
        err = growattInverterTaskStart(g_orchestratorCtx.bmsQueue);
        if (err != ESP_OK) {
            ESP_LOGW(EXAMPLE_TAG,
                     "JKBMS->CAN Growatt route failed to start sender (err=0x%x)",
                     (unsigned)err);
            stopJkbmsRs485Tasks();
            orchestratorReset(&g_orchestratorCtx);
            return err;
        ...
        g_orchestratorCtx.canRs485TranslatorActive = true;
        ESP_LOGI(EXAMPLE_TAG,
                 "Orchestrator started JKBMS RS485->CAN Growatt route: BMS(RS485_%u) -> Inverter(%s:%u)",
                 (unsigned)settings->bms_port,
                 canNameByPort(settings->inverter_port),
                 (unsigned)settings->inverter_port);
        return ESP_OK;
    ...
    if (canDalyToRsPylon) {
        ...
        esp_err_t err = dalyCanBmsTaskStart(g_orchestratorCtx.bmsQueue, settings);
        if (err != ESP_OK) {
            ESP_LOGW(EXAMPLE_TAG,
    ...
    if (canDalyToCanPylon) {
        ...
        if (err != ESP_OK) {
            ESP_LOGW(EXAMPLE_TAG,
                     "Daly CAN BMS task failed for CAN->CAN Pylon route (err=0x%x)",
                     (unsigned)err);
            orchestratorReset(&g_orchestratorCtx);
            return err;
        }

        err = pylonInverterTaskStart(g_orchestratorCtx.inverterQueue);
        if (err != ESP_OK) {
            ESP_LOGW(EXAMPLE_TAG,
                     "Pylon CAN inverter task failed for Daly CAN->CAN Pylon route (err=0x%x)",
                     (unsigned)err);
        ...
        BaseType_t ok = xTaskCreate(orchestratorTask,
                                    "orchestrator",
                                    ORCHESTRATOR_TASK_STACK,
                                    &g_orchestratorCtx,
                                    ORCHESTRATOR_TASK_PRIORITY,
                                    &g_orchestratorTaskHandle);
        if (ok != pdPASS) {
            ESP_LOGE(EXAMPLE_TAG, "Failed to create Daly CAN->CAN Pylon orchestrator task");
        ...
        ESP_LOGI(EXAMPLE_TAG,
                 "Orchestrator started Daly CAN->CAN Pylon route: BMS(CAN%u, %lu bps) -> Inverter(CAN%u, 500k)",
                 (unsigned)settings->bms_port,
                 (unsigned long)bridgeProtocolCanBitrate(settings->bms_protocol),
                 (unsigned)settings->inverter_port);
        return ESP_OK;
    }
