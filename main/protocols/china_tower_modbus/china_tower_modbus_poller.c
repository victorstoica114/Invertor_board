...
static uint16_t chinaTowerModbusCrc16(const uint8_t *data, int len)
{
...
static int chinaTowerBuildReadHoldingRegsReq(uint8_t slave,
                                             uint16_t start,
                                             uint16_t count,
                                             uint8_t *out,
                                             size_t outCap)
{
    ...
    const uint16_t crc = chinaTowerModbusCrc16(out, 6);
...
esp_err_t chinaTowerModbusPollerTick(china_tower_modbus_poller_t *poller,
                                     ...
                                     uint32_t periodMs)
{
    if (poller == NULL || g_chinaTowerModbusPollBlocksCount == 0u) {
    ...
    const china_tower_modbus_poll_block_t *block =
        &g_chinaTowerModbusPollBlocks[poller->pollIndex];
    uint8_t req[8];
    int reqLen = chinaTowerBuildReadHoldingRegsReq(poller->slaveAddr,
                                                   block->start,
                                                   block->count,
                                                   req,
                                                   sizeof(req));
    ...
    if (err == ESP_OK) {
        ...
        poller->pollIndex = (poller->pollIndex + 1u) % g_chinaTowerModbusPollBlocksCount;
