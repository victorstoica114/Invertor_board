...
static uint16_t paceModbusCrc16(const uint8_t *data, int len)
{
...
static int paceBuildReadHoldingRegsReq(uint8_t slave,
                                       uint16_t start,
                                       uint16_t count,
                                       uint8_t *out,
                                       size_t outCap)
{
    ...
    const uint16_t crc = paceModbusCrc16(out, 6);
    out[6] = (uint8_t)(crc & 0xFFu);
    out[7] = (uint8_t)((crc >> 8) & 0xFFu);
    return 8;
}

void paceModbusPollerInit(pace_modbus_poller_t *poller,
                          uart_port_t uart,
                          gpio_num_t dirPin,
                          uint8_t slaveAddr)
{
...
esp_err_t paceModbusPollerTick(pace_modbus_poller_t *poller,
                               int64_t nowUs,
                               uint32_t periodMs)
{
    if (poller == NULL || g_paceModbusPollBlocksCount == 0u) {
    ...
    const pace_modbus_poll_block_t *block = &g_paceModbusPollBlocks[poller->pollIndex];
    uint8_t req[8];
    int reqLen = paceBuildReadHoldingRegsReq(poller->slaveAddr,
                                             block->start,
                                             block->count,
                                             req,
                                             sizeof(req));
    ...
    if (err == ESP_OK) {
        ...
        poller->pollIndex = (poller->pollIndex + 1u) % g_paceModbusPollBlocksCount;
